...
var QueryBuilder = function () {
  ...
  */
	function QueryBuilder(query, table, tableAlias, parentTable) {
		...
		['where', 'orderBy', 'select', 'toSQL', 'orderByRaw', 'then', 'tap', 'catch', 'map', 'bind', 'reduce', 'groupBy', 'gropuByRaw', 'whereRaw', 'whereNot', 'whereIn', 'whereNotIn', 'whereNull', 'whereNotNull', 'whereExists', 'whereNotExists', 'whereBetween', 'whereNotBetween'].forEach(function (fn) {
			_this[fn] = function () {
   ...
   */
		...
		value: function _parseJoinArguments(joinType, joinTableName, alias, attrs, opt, optFn) {
		...
		value: function _$join() {
			var _parseJoinArguments$a = this._parseJoinArguments.apply(this, arguments),
			    ...
			    optFn = _parseJoinArguments$a.optFn;
			...
			var _optFn = typeof opt === 'function' ? opt : optFn;
